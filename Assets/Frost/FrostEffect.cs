...
private void Update()
{
    ...
    {
        float freezingVolume = SoundManager.Instance.GetFreezingSoundVolume(); 

        if (freezingVolume >= 0.65f)
        {
            maxFrost = Mathf.Clamp01((freezingVolume - 0.65f) / (1.0f - 0.65f)); 
        }
        else
        {
            maxFrost = 0; 
        ...
        if (maxFrost >= 1)
        {
            SoundManager.Instance.TriggerLoseCondition(); 
        }
    }
}
